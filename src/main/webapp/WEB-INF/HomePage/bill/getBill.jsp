        ...
        <div class="providerAdd">
           <form id="billForm" name="billForm" class="billForm" method="post" action="${ctx}/bill/bill_add">
             ...
             <div class="">
                 ...
                 <input type="text" name="billcode" class="text" id="billCode" value="${bill.billcode}" datatype="*6-15" nullmsg="请填入数据"> 
				 <!-- 放置提示信息 -->
				 
             </div>
             <div>
                 <label for="productName">商品名称：</label>
                 <input type="text" name="productname" id="productName" value="${bill.productname}" datatype="s1-10" nullmsg="请填入数据"> 
				 <font color="red">*</font>
             </div>
             <div>
                 <label for="productUnit">商品单位：</label>
                 <input type="text" name="productunit" id="productUnit" value="${bill.productunit}" datatype="*1-2" nullmsg="请填入数据"> 
				 <font color="red">*</font>
             </div>
             <div>
                 <label for="productCount">商品数量：</label>
                 <input type="text" name="productcount" id="productCount" value="${bill.productcount}" datatype="n" nullmsg="请填入数据"> 
				 <font color="red">*</font>
             </div>
             <div>
                 <label for="totalPrice">总金额：</label>
                 <input type="text" name="totalprice" id="totalPrice" value="${bill.totalprice}" datatype="n" nullmsg="请填入数据"> 
				 <font color="red">*</font>
             </div>
             <div>
                 <label>供应商：</label>
                 <select name="providerId" id="providerId">
                  <c:forEach var="providerList" items="${providerList}">
				   		<option value="${providerList.id}" <c:if test="${bill.providerid eq providerList.id}">selected='selected'</c:if>  >${providerList.proname}</option>
				   </c:forEach>
